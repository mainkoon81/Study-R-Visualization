...
<img src="https://user-images.githubusercontent.com/31917400/35946556-2fda77ae-0c5c-11e8-9d0c-302632b73627.jpg" width="600" height="200" /> 
 ...
 - [1) Third 'categorical' Variable]
   - Scatter of 'friend_count' vs **'age'** vs **'gender'** (multiple categorical variables)
...
> **Reshaping** the dataset:
 - `dcast(data, keep ~ expand, value.var, fun.aggregate)` for HUMAN:  We specify the dataset we are going to change and modify and then we put in a formula. Now, the first part of the formula (the left of the tilde sign), will list the **categorical variables** I want to keep(age). On the right side of the tilde, we list another **categorical variable**(gender) to expand since we want male and female users to have **their own columns** for 'median_friend_count' values in the original dataframe. `value.var` holds the k.e.y m.e.a.s.u.r.e.m.e.n.t.s. or their values. Lastly, list the aggregate function..telling how to cook the values? 
